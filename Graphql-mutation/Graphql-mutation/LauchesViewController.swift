...
//  Graphql-mutation
//
//  Created by Sachin Daingade on 01/01/24.
//

import UIKit
import SpaceXAPI

class LauchesViewController: UITableViewController {
    var rocket: RocketsQuaryQuery.Data.Rocket!
    ...
    private func fetchLaunches() {
        ...
        let query = LaunchesQuery(upcomingFinds: .some(launchFind), pastFind: .some(launchFind))
        NetworkService.shared.apollo.fetch(query: query) {[weak self] result in
            switch result {
            case .success(let values):
                self?.launches = [
                    values.data?.launchesUpcoming?.compactMap{$0?.fragments.launchFragment} ?? [] ,
                    values.data?.launchesPast?.compactMap{$0?.fragments.launchFragment} ?? []
                ]
